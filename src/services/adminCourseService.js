...
class AdminCourseService {
  ...
  async getAllCourses(options = {}) {
    try {
      ...
      const courses = response.data.courses.map((course) => {
        console.log(
          `Mapping course: ${course.title}, backend courseType: ${course.courseType}`,
        );
        return {
          id: course._id,
          title: course.title,
      ...
      console.log(
        "Mapped courses types:",
        courses.map((c) => ({ title: c.title, courseType: c.courseType })),
      );
  ...
  async updateCourse(id, updates) {
    try {
      // Ensure both status fields are consistent if one is provided
      const payload = { ...updates };

      if ("published" in payload) {
        payload.isPublished = payload.published;
      } else if ("isPublished" in payload) {
